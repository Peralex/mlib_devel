...
for i =1:length(gateway_outs)
    gw = gateway_outs{i};
    if regexp(get_param(gw, 'Name'), '(trigger_out)$')
        toks = regexp(get_param(gw, 'Name'), '(trigger_out)$', 'tokens');
...
for i =1:length(gateway_ins)
    ...
    elseif regexp(get_param(gw, 'Name'), '(adc3_data_out)$')
        ...
        set_param(gw, 'Name', clear_name([myname, '_', toks{1}{1}]));	
    elseif regexp(get_param(gw, 'Name'), '(trigger_in)$')
        toks = regexp(get_param(gw, 'Name'), '(trigger_in)$', 'tokens');
        set_param(gw, 'Name', clear_name([myname, '_', toks{1}{1}]));	
    elseif regexp(get_param(gw, 'Name'), '(pps_in)$')
        toks = regexp(get_param(gw, 'Name'), '(pps_in)$', 'tokens');
        set_param(gw, 'Name', clear_name([myname, '_', toks{1}{1}]));	
    else
        error(['Unknown gateway name: ', gw]);
    end
